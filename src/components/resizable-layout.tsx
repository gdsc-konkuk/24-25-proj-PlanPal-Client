...
interface ResizableLayoutProps {
  ...
  onVisibilityChange?: (panel: "left" | "middle" | "right", visible: boolean) => void
}
...
export function ResizableLayout({
  ...
  defaultMiddleWidth = 33.33,
  defaultRightWidth = 33.33,
  minLeftWidth = 15,
  minMiddleWidth = 15,
  minRightWidth = 15,
  leftVisible,
  ...
  const prevVisibilityRef = useRef({ left: leftVisible, middle: middleVisible, right: rightVisible })
  ...
  const visiblePanelCount = [leftVisible, middleVisible, rightVisible].filter(Boolean).length
  ...
  useEffect(() => {
    ...
    if (
      ...
      // 현재 가시성 상태 업데이트
      prevVisibilityRef.current = { left: leftVisible, middle: middleVisible, right: rightVisible }
      ...
      if (leftVisible) setPrevLeftWidth(leftWidth > 0 ? leftWidth : prevLeftWidth)
      if (middleVisible) setPrevMiddleWidth(middleWidth > 0 ? middleWidth : prevMiddleWidth)
      if (rightVisible) setPrevRightWidth(rightWidth > 0 ? rightWidth : prevRightWidth)
  ...
  useEffect(() => {
    const handleResize = (e: MouseEvent) => {
      ...
      if (isResizingLeft && leftVisible && middleVisible) {
        ...
        const leftPanelWidth = ((e.clientX - containerRect.left) / containerWidth) * 100

        // 최소 너비 체크
        if (leftPanelWidth < minLeftWidth) {
          // 최소 너비보다 작아지면 패널 숨기기
          // 단, 최소 1개의 패널은 유지해야 함
          const visibleCount = [true, middleVisible, rightVisible].filter(Boolean).length
        ...
        if (newMiddleWidth < minMiddleWidth) {
          // 최소 너비보다 작아지면 패널 숨기기
          // 단, 최소 1개의 패널은 유지해야 함
          const visibleCount = [leftVisible, true, rightVisible].filter(Boolean).length
          if (visibleCount > 1 && onVisibilityChange) {
      ...
      if (isResizingRight && middleVisible && rightVisible) {
        ...
        if (rightPanelWidth < minRightWidth) {
          // 최소 너비보다 작아지면 패널 숨기기
          // 단, 최소 1개의 패널은 유지해야 함
          const visibleCount = [leftVisible, middleVisible, true].filter(Boolean).length
          if (visibleCount > 1 && onVisibilityChange) {
